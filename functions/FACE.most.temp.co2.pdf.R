fname<-paste("Figs/FACE.most.temp.co2.",
             months(max(ring.means$Date),abbreviate=TRUE),year(max(ring.means$Date)),".pdf",sep="")
pdf(file=fname,width=16,height=8) #save a graph as pdf
...
#plot rainfall
par(new=TRUE)
plot(Rain_mm_Tot~Date,type="h",lwd=2,data=co.rain_means,ylim=c(0,150),col="gray",lend=2,
     ylab="",xlab="",axes=F)
